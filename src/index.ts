...
function getDivisionShortName(division: DivisionDefinition): string {
...
function getDivisionChannelPrefixes(division: DivisionDefinition): string[] {
  return [
...
function buildDivisionChannelDefinitions(division: DivisionDefinition) {
  const prefix = getChannelSlug(getDivisionShortName(division));
  const textChannels = [
    'general',
    'announcements',
...
function isDivisionChannel(channelName: string, divisions: DivisionDefinition[]) {
  const normalized = channelName.toLowerCase();
  return divisions.some((division) =>
    getDivisionChannelPrefixes(division).some((prefix) => normalized.startsWith(`${prefix}-`))
  );
}

async function deleteAllChannels(guild: any) {
...
async function deleteDivisionChannels(guild: any) {
  await guild.channels.fetch();
  ...
  const channelsToDelete = guild.channels.cache.filter((channel: GuildChannel) => {
    return (
      isDivisionChannel(channel.name, divisions) ||
...
function createAdminPanelEmbed(guild: any) {
  return new EmbedBuilder()
    ...
    .addFields(
      { name: 'Delete Channels', value: 'Delete all channels or delete only division channels safely.', inline: false },
      { name: 'Setup Permissions', value: 'Channels and categories created by division setup are configured to allow only the division role and administrators by default.', inline: false },
      { name: 'Command Chain', value: 'Each division gets a dedicated `command-chain` channel for orders and mission coordination.', inline: false }
    )
    .setColor(0x5865f2)
    .setTimestamp();
}
...
async function handleAdminPanelButton(interaction: any) {
...
function isCommandChannelDefinition(channelName: string) {
  return channelName.endsWith('-command-chain');
}

function getCommandChainMessage(division: DivisionDefinition) {
...
function createDivisionChannelTopic(name: string, division: DivisionDefinition) {
  return `${division.displayName} ${name.replace(/-/g, ' ')} channel.`;
}

function isAdminPanelCommand(interaction: any) {
...
function getGuildForInteraction(interaction: any) {
  if (interaction.guild) return interaction.guild;
  if (interaction.guildId) return discordClient.guilds.cache.get(interaction.guildId);
  return undefined;
}

function getGuildTargetName(guild: any) {
...
function makePermissionNote(division: DivisionDefinition) {
...
function getChannelDisplayName(name: string) {
  return name.replace(/-/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase());
}

function isManagedDivisionCategory(channel: GuildChannel, divisions: DivisionDefinition[]) {
  return channel.type === ChannelType.GuildCategory && divisions.some((division) => channel.name.toLowerCase() === buildDivisionCategoryName(division).toLowerCase());
}

function getGuildRoleForDivision(guild: any, division: DivisionDefinition) {
  return guild.roles.cache.find((role: Role) => role.name === buildDivisionRoleName(division));
}

function buildChannelMessageContent(division: DivisionDefinition) {
...
function isDivisionSetupRequest(command: string) {
  return command === 'complete' || command === 'create-roles';
}

function getDivisionChannelCreationNote(division: DivisionDefinition) {
...
function normalizeCustomId(customId: string) {
...
function isButtonAction(action: string) {
  return action.startsWith('adminpanel_');
}

function isDivisionSetupAction(action: string) {
  return action === 'adminpanel_create_division_setup';
}

function getPanelResponse(action: string, guild: any) {
  if (action === 'adminpanel_delete_all_channels') return `Deleting all channels for ${getGuildTargetName(guild)}.`;
  if (action === 'adminpanel_delete_division_channels') return `Deleting division channels for ${getGuildTargetName(guild)}.`;
  return 'Performing admin panel action.';
}

function isChannelOwner(member: GuildMember) {
  return member.permissions.has(PermissionFlagsBits.ManageChannels);
}

function isDivisionManager(member: GuildMember) {
  return member.permissions.has(PermissionFlagsBits.ManageGuild);
}

function isChannelAdmin(member: GuildMember) {
  return member.permissions.has(PermissionFlagsBits.Administrator);
}

function matchesDivisionChannel(channelName: string, division: DivisionDefinition) {
  const normalized = channelName.toLowerCase();
...
function getCommandChainWelcome(division: DivisionDefinition) {
...
function getDivisionCommandChainDescription(division: DivisionDefinition) {
  return `Command chain and coordination channel for ${division.displayName}.`;
}

function createDivisionSetupSummary(division: DivisionDefinition) {
...
function isDivisionChannelInvite(channel: GuildChannel) {
...
function getOverviewChannelName(channelName: string) {
  return channelName.replace(/-/g, ' ');
}

function createCommandChainTitle(division: DivisionDefinition) {
  return `${division.displayName} Command Chain`;
}

function getDivisionChannelTypeDescription(channelType: ChannelType) {
  return channelType === ChannelType.GuildVoice ? 'Voice' : 'Text';
}

function getAdminPanelActionLabel(action: string) {
  if (action === 'adminpanel_delete_all_channels') return 'Delete All Channels';
  if (action === 'adminpanel_delete_division_channels') return 'Delete Division Channels';
  if (action === 'adminpanel_create_division_setup') return 'Create Division Setup';
  return 'Action';
}

function createAdminPanelField(name: string, value: string) {
...
function isAdminPanelMessage(interaction: any) {
  return interaction.customId?.startsWith('adminpanel_');
}

function getCommandChainAnnouncement(division: DivisionDefinition) {
...
function isDivisionChannelName(name: string) {
...
function getAdminPanelButtonStyles() {
  return {
    deleteAll: ButtonStyle.Danger,
    deleteDivision: ButtonStyle.Secondary,
...
function getGuildName(guild: any) {
...
function isAdminPanelCommandName(name: string) {
...
function isAdminPanelReply(interaction: any) {
...
Division role created: ${divisionRole.name}`;
      ...
      for (const definition of channelDefinitions) {
        const existing = guild.channels.cache.find(
          (channel: GuildChannel) => channel.parentId === category.id && channel.name === definition.name
        );
        ...
        if (definition.type === ChannelType.GuildText) {
        ...
        if (definition.type === ChannelType.GuildText && definition.name.endsWith('-command-chain')) {
          try {
